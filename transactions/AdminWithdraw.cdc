...
transaction(senderAccount: Address, amount: UFix64) {
    ...
    // Define references
    let senderVault: &EchoCoin.Vault{EchoCoin.VaultInterface}
    ...
    let adminResource: &EchoCoin.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&EchoCoin.Admin>(from: /storage/AdminStorage)
        ...
        self.senderVault = getAccount(senderAccount)
            ...
            .borrow<&EchoCoin.Vault{EchoCoin.VaultInterface}>()
