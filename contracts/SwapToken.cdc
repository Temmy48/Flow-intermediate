...
// SwapToken contract: Facilitates token swapping between EchoCoin and FlowToken
pub contract SwapToken {
    ...
    // Function to swap tokens between EchoCoin and FlowToken
    pub fun swapTokens(signer: AuthAccount, swapAmount: UFix64) {

        // Borrow EchoCoin and FlowToken vaults from the signer's storage
        let EchoCoinVault = signer.borrow<&EchoCoin.Vault>(from: /storage/VaultStorage)
            ?? panic("Could not borrow EchoCoin Vault from signer")

        let flowVault = signer.borrow<&FlowToken.Vault>(from: /storage/FlowVault)
            ?? panic("Could not borrow FlowToken Vault from signer")  

        // Borrow the Minter capability from EchoCoin
        let minterRef = signer.getCapability<&EchoCoin.Minter>(/public/Minter).borrow()
            ?? panic("Could not borrow a reference to EchoCoin Minter")
        ...
        // Mint new EchoCoins and deposit them into the vault
        let newEchoCoinVault <- minterRef.mintToken(amount: mintedAmount)
        EchoCoinVault.deposit(from: <-newEchoCoinVault)
