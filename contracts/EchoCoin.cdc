...
// EchoCoin contract: Implements a fungible token on the Flow blockchain
pub contract EchoCoin: FungibleToken {

    // Total supply of EchoCoin
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, VaultInterface {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @EchoCoin.Vault
        ...
        // Destroy the vault
        destroy() {
            EchoCoin.totalSupply = EchoCoin.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            EchoCoin.totalSupply = EchoCoin.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&EchoCoin.Minter>(/public/Minter, target: /storage/MinterStorage)
